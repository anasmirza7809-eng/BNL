...
function useGalleryUrls(paths) {
	...
	(0, import_react.useEffect)(() => {
		...
		Promise.all(paths.map((path) => path.startsWith("http") ? Promise.resolve(path) : supabase.storage.from("property-images").createSignedUrl(path, 86400).then(({ data }) => data?.signedUrl ?? ""))).then((r) => {
